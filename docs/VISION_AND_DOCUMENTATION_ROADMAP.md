...
- [ ] **DOC_MAP.md** (или раздел «Документация» в README):
  - По роли: «хочу использовать» → Quick Start, Cookbook, API Overview; «хочу понять дизайн» → Philosophy, System, Scheme, Design Rationale; «хочу реализовывать/расширять» → TODO_*, DONE_*, канон, LLM_API_SUPPORT, MODEL_REUSE, TRAINING_REUSE_AND_API_SCENARIOS, SERIALIZATION, MULTI_ENDPOINT.
  - По уровню: Block/Node → Graph → Pipeline → Stage → World; для каждого — концепт, API, DONE/TODO.
  - По теме: диффузия, генерация мира, LLM, API-провайдеры (любой блок), сериализация, обучение, повторное использование моделей, LoRA и API, multi-endpoint.
