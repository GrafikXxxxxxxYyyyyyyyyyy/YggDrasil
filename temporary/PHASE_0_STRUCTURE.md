...
Корень проекта — каталог, в котором лежат `documentation/`, `philosophy/`, `reference/`, `temporary/` и куда добавляются каталоги кода и тестов.
...
| **reference/** | Старая кодовая база и референс (уже есть). |
...
После добавления тяжёлых зависимостей (torch, diffusers и т.д.) их перечисляют в `dependencies` или в отдельных optional-dependencies (например, `full`, `train`), как в reference/pyproject.toml.
