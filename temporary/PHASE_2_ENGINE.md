...
- Можно было **хранить** граф: узлы (node_id → AbstractGraphNode), рёбра (источник_узел, источник_порт → приёмник_узел, приёмник_порт), внешние входы/выходы.
...
Контейнер, который хранит узлы, рёбра и списки внешних входов/выходов. Движок читает из него данные для валидации, планирования и выполнения. В фазе 2 узел добавляется как готовый экземпляр AbstractGraphNode (создание блока через реестр по block_type — фаза 3).

**Файл:** `yggdrasill/engine/structure.py`.

**Внутреннее состояние:**

- `_nodes: Dict[str, AbstractGraphNode]` — node_id → узел.
...
Проверяет, что структура гиперграфа **реализуема** перед выполнением: все ссылки на узлы и порты корректны, типы на концах рёбер совместимы, обязательные входы покрыты.

**Файл:** `yggdrasill/engine/validator.py`.

**Вход:** объект структуры (Hypergraph), предоставляющий: node_ids, get_node(node_id), get_edges(), get_input_spec(), get_output_spec().
...
1. Все node_id из рёбер (source_node, target_node) присутствуют в узлах.
2. Все port_name в рёбрах соответствуют объявленным портам блоков: source_port — выход источника, target_port — вход приёмника.
3. Типы портов: для каждого ребра `source_port.compatible_with(target_port)` (или оба порта существуют и совместимы по правилам Port).
4. Записи в exposed_inputs и exposed_outputs: node_id существует, port_name — объявленный вход/выход соответствующего узла.
5. Обязательные входные порты: у каждого узла каждый обязательный (non-optional) входной порт имеет хотя бы одно входящее ребро или запись в exposed_inputs (т.е. порт будет заполнен из внешнего входа).

**Опционально:**

- Связность: от внешних входов достижимы узлы внешних выходов (можно предупреждение, а не ошибка).
- Циклы: если граф содержит цикл (SCC размера > 1 или self-loop), в metadata может быть num_loop_steps; иначе — предупреждение.

**Сигнатура:**

```python
def validate(hypergraph: Hypergraph) -> ValidationResult: ...
```

**Тесты (test_validator.py):**

- Граф без ошибок — errors пустой.
- Несуществующий node_id в ребре — ошибка.
- Несуществующий port_name — ошибка.
- Несовместимые типы портов — ошибка.
- Обязательный вход без входящего ребра и без exposed_input — ошибка.
...
7. **Обход по плану:**
   - Для шага ("node", node_id): собрать входы узла из buffer (по get_edges_in(node_id) и по exposed_input для этого узла); если dry_run — не вызывать block.forward, подставить None для выходов; иначе вызвать **node.run(inputs)** — по контракту фазы 1 (AbstractGraphNode.run делегирует в block.run/forward); записать выходы в buffer по исходящим рёбрам; вызвать callbacks с phase "before"/"after".
...
**Минимальный сценарий «запустился»:** в тесте или примере: создать Hypergraph; добавить три узла (AbstractGraphNode с IdentityBlock); add_edge(A.out, B.in), add_edge(B.out, C.in); expose_input(A, "in", "x"), expose_output(C, "out", "y"); run(h, {"x": value}) → {"y": value}. Это подтверждает, что движок и структура работают от входа до выхода.
...
- Вызов узла: node.run(inputs) (контракт фазы 1), а не node.block.forward; run может быть алиасом block.run/forward.
