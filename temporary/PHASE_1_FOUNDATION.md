...
Детальный технический план по **первой фазе** реализации: порты (Port), абстрактный блок (Abstract Base Block), узел гиперграфа (Abstract Graph Node), реестр типов блоков (Registry). Что реализовать, как это должно выглядеть (API, сигнатуры, контракты), как тестировать и как проверять. Канон — единственный источник истины; референс-код в `reference/` — ориентир, не эталон.
...
- **Подблоки:** метод get_sub_blocks() -> Dict[str, AbstractBaseBlock]. По умолчанию возвращает {}. Если переопределён, state_dict/load_state_dict базового класса могут агрегировать/разбирать состояние подблоков по префиксам (см. референс block.py).
...
- В конструктор блока передаётся: block_id = config.get("block_id"), config = {k: v for k, v in config.items() if k not in ("block_type", "type", "block_id")} (или весь config без только type-полей — по соглашению). Референс: передавать rest конфига как config блока; block_id при необходимости вынимать отдельно.
