...
По канону 02 §3.1 каждая абстрактная сущность узла-задачи **одновременно наследует** Abstract Base Block **и** Abstract Graph Node (**двойное наследование**). Block — только данные и вычисления; Node — только положение и связи в графе. В гиперграф кладутся **только абстрактные узлы-задачи** (один объект = Block+Node); обёртки «узел с блоком внутри» нет. Ниже в §4 и §7 — разделение ответственности и реализация абстракций.
...
- **AbstractBaseBlock** отвечает **только за данные и вычисления**: порты (declare_ports), выполнение (forward), идентичность (block_type, block_id), состояние (state_dict). Не знает о гиперграфе и положении в нём.
- **AbstractGraphNode** отвечает **только за положение и связи** в гиперграфе: node_id, участие в гиперрёбрах (связи задаются графом). Не хранит блок и не оборачивает блок. Даёт движку интерфейс get_input_ports(), get_output_ports(), run(inputs), реализуемый обращением к тому же объекту как к блоку (self.declare_ports(), self.forward()), потому что узел-задача наследует оба класса.
