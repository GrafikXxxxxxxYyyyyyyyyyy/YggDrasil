...
Связь с каноном: [Scheme.md](Scheme.md), [World_Level.md](World_Level.md). Обучаемость: [TRAINABILITY_AT_ALL_LEVELS.md](TRAINABILITY_AT_ALL_LEVELS.md). Обучение при повторном использовании моделей и при API-провайдерах (LoRA только к локальным блокам, промпт-адаптеры для API): [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md).
...
Итого: необходимо реализовать **механизм создания LoRA-адаптеров на весь мир**, **заморозки всех весов** базового мира и **обучения только этих адаптеров**, чтобы поддерживать **лёгкие кастомные миры**. При наличии в мире узлов с API-провайдерами к ним не подключается LoRA к весам (весов нет локально); возможны промпт-адаптеры или пост-обработка — см. [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) §4.
