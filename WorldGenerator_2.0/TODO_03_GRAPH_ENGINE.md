...
**Учёт нового канона:** (1) **Модели через API** ([LLM_API_SUPPORT.md](LLM_API_SUPPORT.md)) — граф может содержать узлы в режиме API; исполнитель вызывает block.forward(inputs); контракт графа не меняется. (2) **Повторное использование моделей** ([MODEL_REUSE.md](MODEL_REUSE.md)) — при сборке графа один ref → один экземпляр блока. (3) **Обучение при reuse:** [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) — дедупликация параметров в оптимизаторе, чекпоинт один раз на ref. (4) **Поддержка агентных систем** ([AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md)) — режим выполнения **agent_loop**: граф с узлом-агентом и узлами-инструментами; цикл agent.forward → tool_calls → выполнение инструментов → agent.forward с tool_results до завершения. (5) **Гибкость использования (канон):** движок графа поддерживает классические сценарии (только граф для инференса/обучения, только граф-агент и т.д.) в рамках единого канона; см. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §0.
...
- [ ] **Режим agent_loop:** если в графе есть узел с агентным контрактом (блок возвращает tool_calls), исполнитель поддерживает цикл: вызов agent.forward(inputs); при наличии tool_calls — выполнение соответствующих узлов-инструментов (по таблице tool_id → node_id); передача tool_results в agent.forward; повтор до отсутствия tool_calls или max_steps. Входы графа подаются агенту на первом шаге; выход графа — финальный ответ агента. Состояние агента (память, диалог) сохраняется в state_dict блока. См. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §2.2.
