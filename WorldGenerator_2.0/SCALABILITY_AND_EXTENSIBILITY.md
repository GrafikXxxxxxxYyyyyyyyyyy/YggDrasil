...
| Класс | Описание |
|-------|----------|
| **LLM** (языковые модели) | Текстовые генеративные модели (автодополнение, генерация текста, чат). В графе — как блоки с контрактом портов (вход: промпт/контекст, выход: текст/токены). **Обязательна поддержка режима «только API»:** использование LLM через удалённый API без скачивания и загрузки весов (например, QwenMax, ChatGPT/GPT-5.x, Claude/Opus и т.д.). Подробно: [LLM_API_SUPPORT.md](LLM_API_SUPPORT.md). Используются в цикле мира (Автор, Архитектор, Development of the world — по канону Scheme). |
