...
**Учёт нового канона:** (1) **Модели через API** ([LLM_API_SUPPORT.md](LLM_API_SUPPORT.md)) — этапы могут использовать пайплайны с блоками только через API; state и порядок не зависят. (2) **Повторное использование моделей** ([MODEL_REUSE.md](MODEL_REUSE.md)) — один checkpoint_ref в разных пайплайнах этапа → один экземпляр. (3) **Обучение этапа при API и reuse:** [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) — LoRA только к локальным блокам; для API-узлов — промпт-адаптеры и т.д. (4) **Поддержка агентных систем** ([AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md)) — этап может быть реализован как **этап-агент**: пайплайн/граф с узлом-агентом и инструментами; контракт по state сохраняется (state ↔ входы/выходы пайплайна). (5) **Гибкость использования (канон):** этап поддерживает классические сценарии (этап-агент, этап как часть мира и т.д.) в рамках единого канона; см. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §0.

### Пункты к реализации на этапе 05 (новый канон)

- [ ] **Пул при сборке этапа:** при AddPipeline и Stage.from_config: пайплайны/графы с общим checkpoint_ref должны использовать один экземпляр модели (пул на уровне этапа или наследование от пайплайна/графа).
- [ ] **Контракт по state и execution_condition:** реализовать по [Scheme.md](Scheme.md); этап с API-пайплайнами выполняется так же (state течёт по маппингу).
- [ ] **Обучение этапа:** при trainable_parameters() дедупликация по блокам; при обучении этапа с API-узлами — только локальные блоки и локальные адаптеры (промпт-адаптер, пост-обработка) по [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) §4.
- [ ] **Этап-агент:** пайплайн этапа может содержать граф с агентом и инструментами; run(stage, state) маппит state во входы пайплайна (в т.ч. контекст для агента), вызывает pipeline.run (с agent_loop внутри графа), маппит выходы в state. Этапы Автор, Философ, Архитектор, Development of the world могут быть реализованы как агентные пайплайны. См. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §2.4.
