...
**Учёт нового канона:** (1) **Модели через API** ([LLM_API_SUPPORT.md](LLM_API_SUPPORT.md)) — узлы пайплайна (графы) могут содержать блоки в режиме API; пайплайн не различает по контракту. (2) **Повторное использование моделей** ([MODEL_REUSE.md](MODEL_REUSE.md)) — два графа-узла с одним checkpoint_ref → один экземпляр модели в памяти. (3) **Обучение при reuse:** [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md). (4) **Поддержка агентных систем** ([AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md)) — узел пайплайна может быть графом-агентом (граф с agent_loop); выполнение узла = run графа с агентным циклом; при необходимости инструменты агента — другие графы-узлы пайплайна (контракт sub_call или документированная схема). (5) **Гибкость использования (канон):** пайплайн может использоваться в классических сценариях (комбинированный язык+диффузия, только агенты и т.д.) в рамках единого канона; см. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §0.

### Пункты к реализации на этапе 04 (новый канон)

- [ ] **Пул при добавлении графов:** при add_graph(graph_or_config) и при Pipeline.from_config: если граф (или графы внутри) ссылаются на тот же checkpoint_ref/model_id, что и другой узел пайплайна, не создавать вторую копию — использовать общий пул (на уровне пайплайна или передавать пул при сборке); в памяти один экземпляр модели на один ref.
- [ ] **Сериализация и загрузка:** при to_config/state_dict сохранять графы и веса с учётом уникальных ref (один раз на ref); при load_from_checkpoint загружать веса в общие блоки один раз.
- [ ] **trainable_parameters() с дедупликацией:** при pipeline.trainable_parameters() объединять параметры графов с дедупликацией по блокам (один блок — один раз в списке параметров для оптимизатора). См. [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md).
- [ ] **Граф-агент как узел пайплайна:** при add_graph допускать граф, содержащий узел-агент и инструменты; run такого узла = run графа в режиме agent_loop. Контракт пайплайна (входы/выходы) задаётся по входам/выходам графа (например, prompt → финальный ответ агента). См. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §2.3.
