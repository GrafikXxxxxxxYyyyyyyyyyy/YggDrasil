...
**Единый канон проекта:** [CANON.md](CANON.md). **Гиперграф как движок:** вся структура фреймворка устроена как **гиперграф** ([HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md)); граф уровня «задача» — представление с рёбрами размера 2; при необходимости поддерживаются гиперрёбра большей арности и **гиперссылки** для связывания контекстов.

Описание **уровня графа** как первого уровня, на котором система **исполняется** ([Abstract_Task_Nodes.md](Abstract_Task_Nodes.md) — узлы-задачи). Граф — это **одна цельная задача** (text-to-image, image-to-video, upscale, распознавание и т.д.); граф **состоит из абстрактных узлов-задач** (Backbone, Solver, Codec, Conditioner, Tokenizer и т.д.), собирается через **Graph.AddNode(...)**, узлы автоматически понимают свою роль. Выше графа — **пайплайн** ([Pipeline_Level.md](Pipeline_Level.md)), **этап** ([Stage_Level.md](Stage_Level.md)), **World** ([System.md](System.md)); на всех уровнях — модель гиперграфа и поддержка гиперссылок.
