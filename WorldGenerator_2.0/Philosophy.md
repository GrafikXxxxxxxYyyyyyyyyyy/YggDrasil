...
**Уровень графа** — первый **исполняемый** уровень; **один граф = одна цельная задача** (text-to-image, image-to-video, upscale и т.д.). Сборка через Graph.AddNode(...), узлы знают свою роль. [Graph_Level.md](Graph_Level.md). **Уровень пайплайна** — граф, **узлами которого являются другие графы**; для **комбинированных задач** (текст → картинка → апскейл → видео). [Pipeline_Level.md](Pipeline_Level.md). **Уровень этапа (Stage)** — **этап есть граф пайплайнов** (узлы этапа = пайплайны; пайплайн может состоять из одного графа). Задаёт шаг в цикле (Философ, Автор, Среда, Архитектор, Творец), контракт по state. [Stage_Level.md](Stage_Level.md). **Уровень мира (World)** — **мир есть граф этапов** ([World_Level.md](World_Level.md)); узлами мира являются этапы, по рёбрам передаётся state; в каноне — циклический граф. *Дополнительно — в [Graph_Stage_Level.md](Graph_Stage_Level.md), [Level_Hierarchy.md](Level_Hierarchy.md) при наличии.*

---

## 8. Верхний уровень: World (Мир)

**World (Мир)** — **граф этапов** ([World_Level.md](World_Level.md)): узлами мира являются этапы, по рёбрам передаётся state. **Циклический граф из пяти звеньев** (каждое звено — этап): Философ → Автор → Среда → Архитектор → Творец:
