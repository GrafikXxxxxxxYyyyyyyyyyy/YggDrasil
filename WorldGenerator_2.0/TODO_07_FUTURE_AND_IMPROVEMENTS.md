...
**Учёт нового канона:** **Гиперграф как движок:** вся архитектура устроена как **гиперграф**; **крупномасштабные гиперссылки** для связывания контекстов и взаимодействия — [HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md). **End Worlds:** поддержка фиксированных, неизменяемых миров; к ним можно привязываться; объектный мир обогащает контекст и истории, End World не меняется — [END_WORLDS.md](END_WORLDS.md). Модели через API и повторное использование: [LLM_API_SUPPORT.md](LLM_API_SUPPORT.md), [MODEL_REUSE.md](MODEL_REUSE.md), [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md). **Агентные системы:** [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) (§0). **Уровень Вселенной:** гиперграф миров, взаимодействие между мирами и сущностями, End Worlds, кросс-мировое влияние — [EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md](EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md). При реализации: один ref/model_id → один экземпляр.

### Пункты к реализации на этапе 07 (новый канон и расширения)

- [ ] **Реализовать обязательные возможности:** модели через API (любой блок), пул повторного использования на всех уровнях, обучение с дедупликацией параметров и сценарии из [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) — до внедрения остальных расширений ниже.
- [ ] **VLM, видео, аудио, мультимодальность:** при добавлении новых типов блоков поддерживать для них режим API (если провайдер даёт API) и повторное использование по model_id/ref.
- [ ] **Бэкенды чекпоинтов (torch, safetensors):** при сохранении/загрузке учитывать пул ref — один ref → один файл/запись в чекпоинте.
- [ ] **Multi-endpoint, async, стриминг:** контракт графа/пайплайна не меняется; при развёртывании узлов на эндпоинты и при API-блоках соблюдать [LLM_API_SUPPORT.md](LLM_API_SUPPORT.md) и [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md) (обучение только локальных частей).
- [ ] **Агентные системы (обязательная возможность):** полная поддержка по [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md): блок-агент (state, tool_calls/tool_results), роль Agent и узлы-инструменты, режим agent_loop в исполнителе графа, граф-агент как узел пайплайна/этапа, мир с агентными этапами. При добавлении VLM, видео и т.д. — агент может использовать их как инструменты.
...
### 1.6 LLM через API (без локальных весов) — обязательная возможность

- [ ] **Полная поддержка LLM по API:** фреймворк должен поддерживать **использование больших языковых моделей исключительно через удалённый API** — без скачивания и без загрузки весов для инференса, без локальной инициализации модели. Узел-задача LLM может быть сконфигурирован как клиент API (провайдер, model_id, endpoint_url при необходимости, учётные данные из окружения или секретного хранилища). При run инференс выполняется запросом к API; контракт портов (вход: промпт/контекст, выход: текст/токены) тот же, что и у локального LLM-блока. Подробно: [LLM_API_SUPPORT.md](LLM_API_SUPPORT.md).
- [ ] **Примеры моделей и провайдеров:** поддержка моделей класса Qwen (QwenMax и др.), OpenAI (ChatGPT, GPT-4, GPT-5.x и т.д.), Anthropic (Claude, Opus 4 и т.д.) и любых LLM, доступных по документированному API. Конкретная модель задаётся в конфиге (например, `model_id`); реализация — блоки-адаптеры под OpenAI-совместимый и иные форматы API.
- [ ] **Интеграция с графом, пайплайном и миром:** LLM в режиме API регистрируется в реестре как узел-задача (например, `llm/api`, `llm/openai`, `llm/anthropic`); граф и пайплайн собираются без изменений; этапы мира (Автор, Философ, Архитектор и т.д.) могут использовать только API-LLM без локальных весов.

### 1.7 Повторное использование одной и той же модели (без дублирования в памяти)

- [ ] **Механизм повторного использования:** когда несколько узлов или графов используют **один и тот же чекпоинт** (одну и ту же модель) — например, в пайплайне два графа: text-to-image и image-to-image с одним и тем же backbone, или несколько узлов с одной LLM — модель не должна создаваться в двух копиях и загружаться в память дважды. Должен использоваться **один экземпляр** на один идентификатор чекпоинта (checkpoint_ref, path, model_id для API), чтобы не тратить лишнюю память. Подробно: [MODEL_REUSE.md](MODEL_REUSE.md).
- [ ] **Для всех типов моделей:** диффузия (backbone, codec, conditioner), LLM (локальные и по API), VLM и любые блоки с весами. Для API-моделей — один клиент/пул на один model_id/endpoint.
- [ ] **Реализация:** пул или кэш «checkpoint_ref → экземпляр блока» при сборке графа/пайплайна/этапа/мира; при добавлении узла или графа с уже известным ref возвращается ссылка на существующий экземпляр. Сериализация и загрузка из чекпоинта совместимы с повторным использованием (один ref в конфиге — один экземпляр после загрузки).
- [ ] **Обучение при повторном использовании и при API:** коллизии (дублирование параметров в оптимизаторе, градиенты с нескольких мест), LoRA при общем блоке, обучение мира с API-узлами (промпт-адаптеры, пост-обработка), сценарии и обходы — подробно: [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md).

### 1.8 Уровень Вселенной (расширение): граф миров и кросс-мировое влияние

- [ ] **Вселенная как гиперграф миров:** расширение выше уровня мира — **Universe** как **гиперграф** ([HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md)), вершины — миры (world_id → World), гиперрёбра — связи между мирами; payload_spec (артефакты, контекст, события, сущности). Взаимодействие между мирами и между сущностями; поддержка **End Worlds** и **гиперссылок**. Подробно: [EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md](EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md).
- [ ] **Агентные системы на уровне вселенной:** обмен данными между мирами; сущности могут «переходить» из мира A в мир B; кросс-мировое влияние. См. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §0.4, [EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md](EXPANSION_UNIVERSE_GRAPH_OF_WORLDS.md) §1.1–1.2.
- [ ] **Гиперграф как движок и гиперссылки:** при реализации уровней (граф, пайплайн, этап, мир, вселенная) закладывать модель **гиперграфа** (гиперрёбра произвольной арности, не только пары); поддержка **крупномасштабных гиперссылок** в контрактах и сериализации для связывания контекстов и взаимодействия. См. [HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md).
- [ ] **End Worlds:** поддержка **фиксированных миров** (только чтение, неизменяемые); объектные миры могут ссылаться на них по гиперссылке и обогащать свой контекст и истории персонажей; End World не изменяется. Пример: книга как End World. См. [END_WORLDS.md](END_WORLDS.md).
