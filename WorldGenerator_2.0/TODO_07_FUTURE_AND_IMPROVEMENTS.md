...
**Учёт нового канона:** в этом TODO уже включены разделы **1.6 Модели через API (без локальных весов)** и **1.7 Повторное использование одной и той же модели** со ссылками на [LLM_API_SUPPORT.md](LLM_API_SUPPORT.md) и [MODEL_REUSE.md](MODEL_REUSE.md). Поддержка API — для **любого типа блока** (LLM, VLM, эмбеддинги, генерация по API и т.д.). Обучение при reuse и API: [TRAINING_REUSE_AND_API_SCENARIOS.md](TRAINING_REUSE_AND_API_SCENARIOS.md). При реализации любых расширений: один ref/model_id → один экземпляр.
