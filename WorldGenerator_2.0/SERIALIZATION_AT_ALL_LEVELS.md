...
Итого: от блока до мира **веса сериализуются на каждом уровне**, и на **каждом уровне** возможна **загрузка из чекпоинта**. При этом один и тот же чекпоинт может использоваться в нескольких узлах/графах без дублирования весов в памяти — см. [MODEL_REUSE.md](MODEL_REUSE.md).
