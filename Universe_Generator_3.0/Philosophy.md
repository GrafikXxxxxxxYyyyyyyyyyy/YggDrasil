...
**Статус документа.** Философия проекта **полностью завершена** и **ни при каких условиях не подлежит изменению**. Она **замкнута с обеих сторон** (начальная граница снизу, вселенная и мета-пространство сверху). Настоящий документ является **якорем** для построения всей дальнейшей архитектуры и **единственным идеологическим ориентиром (библией)** всего фреймворка: канон, схема и код должны соответствовать философии; при противоречии пересматривается канон или реализация, а не философия. Философия задаёт **ориентацию** для всего проекта — от фундамента до вселенной.

**Архитектурный канон:** папка `../WorldGenerator_2.0/` — референс архитектуры; данный документ извлекает и систематизирует и расширяет философию, заложенную в каноне, в единый связный текст.
...
**Устойчивость документа.** Данный текст философии — **неизменяемый якорь** и **ориентация для всего фреймворка**. Реализации, канон и конкретные форматы могут эволюционировать, но сам документ Philosophy **не подлежит правкам ни в какой форме**. Принципы (два начала, синтез, гиперграф, Lego, граница, кастомизируемость без изменений движка и т.д.) зафиксированы окончательно; в Philosophy **не вносятся изменения** — ни под частные случаи, ни под эволюцию канона или кода. Вся дальнейшая архитектура строится **от этого якоря**; канон и код могут меняться только **в рамках** философии. Философия используется как **единственная библия** проекта, относительно которой определяется «соответствие» всему остальному.
