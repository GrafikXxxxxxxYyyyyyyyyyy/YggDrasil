...
- [ ] **Узлы-задачи Agent и Tools:** абстрактный тип Agent (контракт: context, tool_results → response, tool_calls); регистрация реализаций `agent/llm`, `agent/api`, `agent/react` и т.д. Любой блок с стабильным контрактом портов может выступать инструментом; в графе — сопоставление tool_id с node_id для исполнителя. Role rules: агент может соединяться с узлами-инструментами по таблице (agent → tool по вызову). См. [AGENT_SYSTEMS_SUPPORT.md](AGENT_SYSTEMS_SUPPORT.md) §2.2.
