...
**Единый канон проекта:** [CANON.md](CANON.md). **Единый источник правды** по операционной модели мира: цикл, state, обновление и развитие; идеальные задачи этапов (§1.1). Философская основа — [Philosophy.md](Philosophy.md). Уровень мира — [World_Level.md](World_Level.md).

---

## 1. Мир: непрерывная цепь крупномасштабных переходов

**Мир (World)** — **непрерывная цепь крупномасштабных переходов** (transitions): Философ → Автор → Среда → Архитектор → Творец. Каждый из этих компонентов есть именно **переход** — **фаза мира**, **преобразование state**. В структурном представлении мир — **граф переходов** (узлы = переходы, по рёбрам передаётся state); см. [World_Level.md](World_Level.md). На каждом шаге выполняется **в одном и том же порядке**. Двух направлений графа нет.

### 1.0 Переход (Transition): уровень глубоких метаморфоз

- **Переход (transition)** в широком смысле — это **фундаментальное изменение состояния системы**: **метаморфоза**. Один объект превращается в другой; материя — в идею, идея — в материю; глубокие, всеобъемлющие метаморфозы. **Уровень перехода** — это уровень именно таких преобразований: не просто передача данных, а качественное превращение state (например, артефакты → идеализированное описание; описание → конфигурация; конфигурация → новые артефакты; state → обновлённый мир).
- В каноне термин **«этап» (stage)** в контексте мира следует понимать как **переход**: каждое звено цикла мира (Философ, Автор, Среда, Архитектор, Творец) — это **переход**, реализуемый на уровне Stage (граф пайплайнов). То есть **stage** в широком смысле заменяется/уточняется понятием **transition (переход)**; уровень перехода — уровень метаморфоз состояния.

**Порядок выполнения (цепочка переходов):**

```
  Философ  →  Автор  →  Среда  →  Архитектор  →  Творец  →  …
```

Между переходами передаётся **state**; каждый переход читает и/или дополняет state, осуществляя метаморфозу (преобразование) состояния.
...
## 1.1 Идеальные задачи переходов (канон ролей)

Семантика ролей в мире: что каждый **переход** делает в идеальном представлении цикла (каждый переход — метаморфоза state).

| Переход (роль) | Идеальная задача |
|------|------------------|
| **Философ (Philosopher)** | **Описание и идеализация** всего того, что приходит в мир как материальное. Дать описание **новых сущностей** и **нового способа** их представления на уровне смыслов и логики. Вход — материальные артефакты (блок 3); выход — идеализированное описание (блок 4). |
| **Автор (Author)** | **Глубокий анализ** идей Философа и описание новых существ в рамках **мирового контекста** и хода истории мира. Сводит идеальное описание (блок 4) в нарратив «что реально произошло в мире» (блок 5). |
| **Среда (Environment)** | **Управление миром**: редактирование контекста мира и **генерация следующих шагов** развития мира. Два узла: (1) **World update** — применение state к миру, обновление контента; (2) **Development of the world** — порождение нового state (блок 1) с учётом Action или по текущему состоянию мира. |
| **Архитектор (Architect)** | **Интерпретация** того, как мир должен развиваться, и **перевод** идеологической структуры развития мира в **технически доступную структуру** для реализации изменений. Подготовка **технической документации** для реализации этих изменений. Вход — блок 1; выход — конфигурация генерации (блок 2). |
| **Творец (Creator)** | **Прямая реализация** технических изменений и **создание** новых образов, структур и материальных объектов в мире. По конфигурации (блок 2) генерирует артефакты (блок 3). |

**Цикл в идеальном представлении:** новые материальные объекты (блок 3) снова попадают к Философу и идеализируются на уровне представлений, смыслов и логики (блок 4). Автор, получая эти идеальные описания, формулирует, как они вписываются в контекст мира и ход его истории (блок 5). Среда применяет контекст к миру и генерирует следующий шаг развития мира (Development of the world → новый блок 1). Цикл повторяется.
