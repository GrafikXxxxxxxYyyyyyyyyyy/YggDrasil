...
**Единый канон проекта:** [CANON.md](CANON.md). Описание системы **с самого основания** — от фундаментальных сущностей до полного цикла мира. Один файл, снизу вверх. **Структура как гиперграф:** вся архитектура устроена как **гиперграф** ([HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md)); граф на каждом уровне — представление с (гипер)рёбрами; поддерживаются **гиперссылки** для связывания контекстов.
...
- **Система как целое** — **гиперграф**, узлы которого — блоки; (гипер)рёбра задают потоки данных и связи между портами; при необходимости — гиперрёбра большей арности и гиперссылки. Содержимое узлов — материя; структура гиперграфа — идея. Подробно: [HYPERGRAPH_AND_HYPERLINKS.md](HYPERGRAPH_AND_HYPERLINKS.md).
...
### 2.7 Уровень перехода (Stage): граф пайплайнов, метаморфоза state

**Переход (transition)** — фундаментальное изменение состояния системы, **метаморфоза** (материя ↔ идея, объект в объект; см. [Scheme.md](Scheme.md) §1.0). **Stage** — уровень, на котором реализуется один такой переход: **Stage — граф пайплайнов** (узлы = пайплайны). Между переходами в цикле мира передаётся state. **Мир — непрерывная цепь переходов:** цикл собран из переходов (Философ, Автор, Среда, Архитектор, Творец), каждое звено — переход, реализуемый одним Stage. Подробно: [Stage_Level.md](Stage_Level.md).
...
## 4. Верхний уровень: мир как цепочка переходов

**Мир (World)** — **непрерывная цепь крупномасштабных переходов** ([World_Level.md](World_Level.md)): узлами мира являются **переходы** (transitions), по рёбрам передаётся state. Каждый переход — метаморфоза состояния (материя ↔ идея; [Scheme.md](Scheme.md) §1.0). В каноне — **циклический граф**. Каждое звено цикла — **переход** (Философ, Автор, Среда, Архитектор, Творец); мир собран из переходов. **Циклический граф из пяти переходов:**
